...
if __name__== "__main__":
    ...
    parser.add_argument('-dl', '--discriminator_loss', action='store', dest='discriminator_loss', default='rel', choices=['wass','rel'], help='Discriminator loss function to be used for the training')
    parser.add_argument('-dla', '--discriminator_loss_activation', action='store', dest='discriminator_loss_activation', default='log-sigm', choices=['none', 'sigmoid', 'log-sigm', 'tanh', 'bi-log'], help='Discriminator loss function to be used for the training')
    parser.add_argument('-dlw', '--discriminator_loss_weight', action='store', dest='discriminator_loss_weight', default='1e-5', type=float, help='Weight of the discriminator loss function to be used for the training GAN. Generator loss weight is considered to be equal 1')
    ...
    parser.add_argument('--upscaler_block_filters', action='store', dest='upscaler_block_filters', default='64', type=int)
    
    parser.add_argument('--a_block_type', action='store', dest='a_block_type', default='3path', choices=['2path','3path'])
    parser.add_argument('--a_block_num', action='store', dest='a_block_num', default='4', type=int)
    parser.add_argument('--a_block_kernel', action='store', dest='a_block_kernel', default='3', type=int)
    ...
    parser.add_argument('--c_block_kernel', action='store', dest='c_block_kernel', default='3', type=int)
    ...
    if model_prefix == 'auto':
        model_prefix = "gan3_" + values.generator_model + "_" + values.content_loss + "_" + values.discriminator_model + "_" + values.discriminator_loss + '_' + values.discriminator_activation + '_' + values.discriminator_loss_activation + ("_x%d" % downscale_factor)
    ...
    elif values.generator_model == 'inc-resnet':
        upscaler = make_upscaler_incep_resnet(
            output_image_shape, filters = values.upscaler_block_filters, upscale_factor = downscale_factor,
            a_block_type = values.a_block_type, a_block_num = values.a_block_num, a_block_kernel = values.a_block_kernel,
            b_block_type = values.b_block_type, b_block_num = values.b_block_num, b_block_kernel = values.b_block_kernel,
            c_block_type = values.c_block_type, c_block_num = values.c_block_num, c_block_kernel = values.c_block_kernel
        )
    ...
    if values.discriminator_model == 's512':
    ...
    if values.discriminator_loss == 'wass':
        discriminator_losses = lambda: WassersteinLosses()
    elif values.discriminator_loss == 'rel':
        discriminator_losses = lambda: RelativisticLosses(loss_activation = values.discriminator_loss_activation)
    
    # setting up the model for training
    gen_train, disc_train, gan_train = make_and_compile_gan2(
        ...
        discriminator_losses = discriminator_losses, discriminator_loss_weight = values.discriminator_loss_weight
    )
    ...
    for b in tqdm(range(values.number_of_batches), desc = 'Batch'):
        ...
        disc_fake_y = -np.ones(3 * values.batch_size)
        
        # training
        loss_disc = disc_train.train_on_batch([image_batch_hr, image_batch_gen], disc_fake_y)
        loss_gan, loss_gan_gen, loss_gan_disc = gan_train.train_on_batch([image_batch_lr, image_batch_hr], [image_batch_hr, disc_fake_y])
